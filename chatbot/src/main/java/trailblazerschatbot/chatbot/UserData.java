...
public class UserData {
	...
	public List<String> getClothingSuggestion() {
		List<String> responses = new ArrayList<>();
		for (int i = 0; i < weatherData.size(); i++) {
			responses.add("Its " + weatherData.get(i).get(0) + "°C outside. The condition is '"+weatherData.get(i).get(2)+"'. "
					+ WeatherAPIModule.getClothing(weatherData.get(i).get(2)) + " for " + location[i][0] + " on "
					+ location[i][1] + ".");
		}
		return responses;
	}
